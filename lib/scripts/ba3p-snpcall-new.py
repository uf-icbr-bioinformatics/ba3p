...
def sickleBowtie(self):
    for smp in self.sc.samples:
        ...
        smp['sickle1'] = ACT.setFileExt(in1, ".sickle.fastq", remove=[".fastq", ".gz"])
        smp['sickle2'] = ACT.setFileExt(in2, ".sickle.fastq", remove=[".fastq", ".gz"])
        smp['bamfile'] = smp['name'] + ".bam"
        this = ACT.submit("sickle.qsub {} {} {} {}".format(fastq1, fastq2, smp['sickle1'], smp['sickle2']))
        # print "Submitted: " + this
        ACT.submit("bowtie2-pe.qsub {} {} {} {}".format(self.btidx, smp['sickle1'], smp['sickle2'], smp['bamfile']), done="bowtie2.@.done", after=this)
    ACT.wait(('bowtie2.@.done', self.sc.nsamples))
    ...
    def postBowtie(self):
        ...
        for smp in self.sc.samples:
            ACT.submit("gzip.qsub {}".format(smp['sickle1']))
            ACT.submit("gzip.qsub {}".format(smp['sickle2']))
            bamfile = smp['bamfile']
            if checkFile(bamfile, step="bowtie2"):
                smp['alignedReads'] = ACT.shell("module load bamtools; bamtools count -in {}".format(bamfile))

    def doGATK(self):
        for smp in self.sc.samples:
            name = smp['name']
            bamfile = smp['bamfile']
            smp['picard'] = ACT.setFileExt(bamfile, ".picard.bam")
            smp['dupmark'] = ACT.setFileExt(bamfile, ".dupmark.bam")
            smp['metrics'] = name + ".metrics.txt"
            smp['realigned'] = name + ".realigned.bam"
            smp['fixmates'] = name + ".fixmates.bam"

            job1 = ACT.submit("picard.qsub AddOrReplaceReadGroups I={} O={} RGID=ID_{} RGLB=LB_{} RGPL=ILLUMINA RGPU=PU_{} RGSM=SM_{}".format(bamfile, smp['picard'], 
                                                                                                                                              name, name, name, name))

            job2 = ACT.submit("picard.qsub MarkDuplicates I={} O={} M={} REMOVE_DUPLICATES=true ASSUME_SORTED=true".format(smp['picard'], smp['dupmark'], smp['metrics']), 
                              after=job1)
            
            job3 = ACT.submit("gatk-realigner.qsub {} {} {}".format(self.reference, smp['dupmark'], smp['realigned']), after=job2)
            
            ACT.submit("picard.qsub FixMateInformation I={} O={} SORT_ORDER=coordinate".format(smp['realigned'], smp['fixmates']), after=job3, done="picard.@.done")
        ACT.wait(("picard.@.done", self.sc.nsamples))
